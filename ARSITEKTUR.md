...
```mermaid
graph TD
    A["SISKEUDES (Keuangan)"] --> B["Laporan Excel Realisasi"]
    B --> C["DASBOR DESA (Agregasi &amp; Visualisasi)"]
    D["Prodeskel (Kependudukan)"] --> C
    E["SIMDABUMAS (Layanan)"] --> C
    C --> F["Dashboard Publik"]
    C --> G["Laporan Triwulan"]
```
